<template>
  <div>
    <el-card>
        ...
        <el-table :data="tableData" border  ref="multipleTable"  @selection-change="handleSelectionChange" size="mini" stripe style="margin-top: 15px;">
          ...
          <el-table-column fixed="right" label="操作" width="150">
            <template slot-scope="scope">
              ...
              <el-tooltip class="item" effect="dark" content="更改订单状态" placement="top">
                <el-button type="info" class="fa fa-check-square-o" size="mini" @click="setStatus(scope.row)" circle ></el-button>
              </el-tooltip>
        ...
        <el-form ref="form" :model="form" :rules="formrules" label-width="100px" size="mini">
            ...
            <el-col :xs="12" :sm="12" :md="12" :lg="12" :xl="12">
              <el-form-item label="省市县" prop="province">
                <el-select v-model="form.province" placeholder="請選取縣市" @change="changeProvince">
                  <el-option
                    v-for="(item,items) in form.citys"
                    :key="items"
                    :label="item.city"
                    :value="items"
                  ></el-option>
          ...
          <el-form-item label="订单状态" prop="trade_status" style="font-weight: bold;">
            <el-select v-model="form.trade_status" placeholder="请选择" disabled>
              <el-option
                v-for="item in form.tradeStatusOptions"
                :key="item.value"
                :label="item.label"
                :value="item.value"
              ></el-option>
          ...
          <el-row
            :gutter="0"
            v-for="(o,list) in form.orderTableData"
            :key="list"
            style="background-color: #eaeaea;border-radius: 10px; padding: 12px 35px 3px 1px;margin-bottom: 5px; "
          >
      ...
      <!-- ***** 更改订单状态 ****** -->
      <el-dialog title="更改订单状态" :visible.sync="toSetStatusVisible" width="30% !important" center>
        <span class="orderlistshenhe">！更改订单状态</span>
        <span slot="footer" class="dialog-footer">
...
export default {
  ...
  data() {
    return {
      ...
      toSetStatusVisible: false, //更改订单状态弹框
      searchOptions: {
        // 地区
        orderAddressOption: [
          {
  ...
  mounted(){
    this.getList();
    this.form.citys = this.tiwanCity();
  },
  methods: {
    AxiosReturn,
    tiwanCity,
